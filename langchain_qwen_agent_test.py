...
# Tavily 搜索工具 API Key（可选）
# 建议通过环境变量 TAVILY_API_KEY 配置，不要硬编码到仓库里。
...
class Qwen25LLM(BaseLLM):
    ...
    def __init__(self):
        ...
        # 初始化Tokenizer（Qwen需特殊设置padding_side为left）
...
def main():
    ...
    # 用自定义模板创建 Prompt
    # create_react_agent 需要的变量一般包含：input, agent_scratchpad, tools, tool_names
    # 这里我们额外加入 chat_history 以配合记忆。
